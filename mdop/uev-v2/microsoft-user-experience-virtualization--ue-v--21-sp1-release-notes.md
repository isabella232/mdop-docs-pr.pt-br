...
---
# Notas de versão da Virtualização de Experiência do Usuário (UE-V) 2.1 SP1 da Microsoft


Para pesquisar as notas de versão do Microsoft User Experience Virtualization 2,1 SP1, pressione Ctrl + F.
...
Esta seção contém notas de versão para o User Experience Virtualization 2,1 SP1.
...
O UE-V inclui um programa de instalação localizado para o UE-V Agent e o UE-V Generator. Esses arquivos MSI ainda estão disponíveis, mas a interface do usuário é minimizada e o MSI é exibido somente em inglês. Apesar de o arquivo estar em inglês, o programa de instalação instala todos os idiomas com suporte durante a instalação.
...
SOLUÇÃO alternativa: ao migrar do UE-V 1 para o UE-V 2, é provável que você tenha computadores executando a versão anterior do agente, crie um catálogo do UE-V 2. x separado para dar suporte ao agente e modelos do UE-V 2. x.

### Atraso de logoff do UE-V

Ocasionalmente, no logoff, o UE-V leva muito tempo para sincronizar as configurações. Geralmente, isso se deve a uma rede de alta latência ou uso incorreto do sistema de arquivos distrubuted (DFS).
Para suporte a DFS, consulte a [declaração de suporte da Microsoft em torno de dados de perfil de usuário replicados](https://support.microsoft.com/kb/2533009) para obter mais detalhes.

SOLUÇÃO alternativa: Iniciando com HF03, uma nova chave do registro foi introduzida a chave do registro a seguir fornece um mecanismo pelo qual o atraso de logoff máximo pode ser especificado \\Software\\Microsoft\\UEV\\Agent\\Configuration\\LogOffWaitInterval

Consulte [configurações do UE-V Registry](https://support.microsoft.com/kb/2770042) para obter mais detalhes

## Hotfixes e artigos da base de dados de conhecimento para UE-V 2,1 SP1


Esta seção contém hotfixes e artigos da KB para o UE-V 2,1 SP1.
