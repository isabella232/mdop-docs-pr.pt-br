...
1.  Inicialize um computador de usuário final usando a imagem de recuperação do DaRT.
    ...
    Você geralmente usará um dos seguintes métodos para inicializar o DaRT para recuperar um computador remoto, dependendo de como você implanta a imagem de recuperação do DaRT. Para obter mais informações sobre como implantar a imagem de recuperação do DaRT, consulte [implantando o DART 10](deploying-dart-10.md).
...
6. Selecione a instalação que você deseja reparar ou diagnosticar e, em seguida, clique em **Avançar**.
   ...
   **Observação**  
   Se o ambiente de recuperação do Windows (WinRE) detectar ou suspeitar que o Windows 10 não foi iniciado corretamente na última vez em que ele foi tentado, o **reparo de inicialização** poderá começar a ser executado automaticamente. Para obter informações sobre como resolver esse problema, consulte [solução de problemas do DART 10](troubleshooting-dart-10.md).
...
10. Clique em **Iniciar**, em **todos os programas**, em **Microsoft DART 10**e, em seguida, clique em **Visualizador de conexão remota do DART**.
